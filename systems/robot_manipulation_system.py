...
class RobotManipulationSystem:
    def __init__(self, control_type: str = "spacemouse", save_dir: str = "./data/", camera_calib_file: str = "/home/dell/maple_control/data/20250829_fruits_and_tray/20250829_173310"):
        # 初始化机器人环境
        ...
        self.camera_envs = [
            RealSenseEnv(camera_name="main_image", serial_number="339322073638", width=1280, height=720, 
                         camera_param=CameraParam(np.array([[908.1308, 0, 655.7268], [0, 910.0818, 395.8856], [0, 0, 1]], dtype=np.float32), 
                                                  np.array([0.1068, -0.2123, -0.0092, 0.0000, 0.0000], dtype=np.float32),
                                                  np.array([1.4102845191955566, -1.6426513195037842, 0.8768366575241089], dtype=np.float32),
                                                  np.array([0.36878958344459534, 0.21233461797237396, 0.6271646618843079], dtype=np.float32))),
        ...
        # self.camera_envs[0].camera_param.load_from_file(camera_name="main_image", file_dir="/home/dell/maple_control/data/pick_up_water_bottle/20250827_221341")
        # self.camera_envs[1].camera_param.load_from_file(camera_name="top_image", file_dir="/home/dell/maple_control/data/pick_up_water_bottle/20250827_221341")
        self.task_info = {
            "name": "default_task",
            "success": False,
    ...
    def reset_for_collection(self):
        ...
        logging.info("Robot Reset Completed.")
            # delta_z = eef_pose[2] - 0.05 * np.random.rand(1)[0] - 0.02
            # action = np.array([0.0, 0.0, -delta_z, 0.0, 0.0, 0.0])
            # success = self.robot_env.step(action, asynchronous=False)
            # if not success:
        ...
        try:
            ...
            logging.warning(f"Failed to open gripper during reset:{e}")
