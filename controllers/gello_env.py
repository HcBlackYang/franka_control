...
class GelloEnv(ControllerEnv):
    def __init__(self, robot_env: RobotEnv, port: str = "/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_FTAC91FI-if00-port0"):
        ...
        self._state = {
            ...
            "gripper": {
                    "target_position": 1.0,  # 目标位置 0=关闭, 1=打开
                    "current_position": 1.0,  # 当前夹爪位置
                },
    ...
    def _update_internal_state(self, hz=200):
        ...
        while not self._stop_monitoring.is_set():
            try:
                ...
                # 使用锁保护状态更新
                with self._state_lock:
                    self._state["joint_angles"] = action[:7]  # 前7个关节角度
                    self._state["gripper_position"] = action[7]  # 最后一个
                    self._state["controller_on"] = True 
                    self._state["timestamp"] = time.time()
                    if action[-1] > 0.5:
                        self._state["gripper"]["target_position"] = 0.0  # 关闭夹爪
                    else:
                        self._state["gripper"]["target_position"] = 1.0  
                    print(self._state)                    
            except Exception as e:
