...
class CameraParam:
    ...
    def estimate_camera_poses(self, image, 
        ...
        使用 ArUco 标定板估计相机位姿
        :param image: 输入图像
        :param board: ArUco 标定板对象，包含标定板信息
        :param camera_matrix: 相机内参矩阵
        :param dist_coeffs: 相机畸变系数
        :param detector: ArUco 标记检测器
    ...
    def draw_cube_and_axis_on_image(self, image, cube_size=0.1):
        ...
        cube_points_3d += np.array([half_size, half_size, 0])  # 将立方体放置在相机坐标系的原点处
        ...
        cv2.line(image, tuple(axis_img_points[0]), tuple(axis_img_points[1]), (0, 0, 255), 2)  # X轴 红色
        cv2.line(image, tuple(axis_img_points[0]), tuple(axis_img_points[2]), (0, 255, 0), 2)  # Y轴 绿色
        cv2.line(image, tuple(axis_img_points[0]), tuple(axis_img_points[3]), (255, 0, 0), 2)  # Z轴 蓝色
        return image
    def draw_trajectory_on_image(self, image, trajectory):
        """
        在图像上绘制手部轨迹
        :param image: 输入图像
        :param trajectory: 手部轨迹列表，每个元素是一个 (x, y, z) 坐标元组
        :param CameraParam: CameraParam 对象，包含相机内参和外参
        :return: 绘制轨迹后的图像
        """
        if self.extrinsic_rvec is None or self.extrinsic_tvec is None:
            raise ValueError("Extrinsic parameters are not set. Please run estimate_camera_poses first.")
        for i in range(len(trajectory)):
            ...
            if x > image.shape[1] or x < 0 or y > image.shape[0] or y < 0:
                continue
            color_ratio = i / max(1, len(trajectory) - 1)  # 避免除零错误
            green_intensity = int(255 * (1 - color_ratio))  # 绿色分量递减
            red_intensity = int(255 * color_ratio)  # 红色分量递增
            color = (0, green_intensity, red_intensity)  # BGR格式：蓝,绿,红
            cv2.circle(image, tuple(img_point1[0][0].astype(int)), 5, color, -1)  # 绘制点
        return image
